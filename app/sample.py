...
def index():
    ...
    games = db.game.find_one({"stage":1})
    ...
    if games != None:
        ...
        body += '<a href="/" class = "btn btn-primary" type = "button"> Start </a>'
    body += '</div>'
    return body
...
def questioning():
    ...
    db.game.update_one({'stage' : 1}, {"$inc" : {"step" : 1}})
...
def answering():
    ...
    if questions[ans_index] == ans:
        db.game.update_one({'stage': 1}, {"$push": {"answer": ans}})
        db.game.update_one({'stage': 1}, {"$inc": {"step": 1}})
        db.game.update_one({'stage': 1}, {"$pop": {"guessing": 1}})
    else:
        db.game.update_one({'stage': 1}, {"$inc": {"fail": 1}})
    return index()
